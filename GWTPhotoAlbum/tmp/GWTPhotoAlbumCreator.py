...
FRAME_W, FRAME_H = 780, 540

selectableResolutions = ((480, 320), (640, 360), (960, 640),  
...
class WebAlbum:
    def __init__(self):
        ...
        self.resolutions = set([sr[0], sr[2], sr[4], sr[5]])
        self.gallery = True
        self.title = ""
        self.subtitle = ""
        self.bottomline = ""
        self.fullscreen = True
        self.filmstrip = False
...
class ImagesCard(Card):
    def __init__(self, parent, webalbum):
        Card.__init__(self, parent)
        self.album = webalbum
        
        label = Label(parent, text="Please add images or image directories:")
        label["bg"] = "#FFFF50"
        label.pack()
        ...
        buttonRow.pack(side=BOTTOM, fill=X)
...
class CaptionsCard(Card):
    def __init__(self, parent, album):
        ...
        label = Label(parent, text="Adjust image captions, if you please:")
        label["bg"] = "#FFFF50"
        label.pack(side=TOP)        
        ...
        lbox.pack(side=TOP, fill=BOTH, expand=1)         
        scrollbar = Scrollbar(lbox, orient=VERTICAL)        
        self.listbox = Listbox(lbox, yscrollcommand=scrollbar.set)
        scrollbar.config(command=self.listbox.yview)
        scrollbar.pack(side=RIGHT, fill=Y)        
        self.listbox.pack(side=TOP, fill=BOTH, expand=1)
        self.listbox.bind("<ButtonRelease-1>", lambda event: self.onSelect())
        ...
        left = Frame(bottom)
        left.pack(side=LEFT)
        self.loadCapJSON = Button(left, text="Load from file...", command=self.loadFromJSON)
        self.loadCapJSON.pack(side=TOP)
        self.saveCapJSON = Button(left, text="Save to file...", command=self.writeToJSON)
        self.saveCapJSON.pack(side=BOTTOM)
        
        self.textedit = Text(bottom)
    ...
    def writeToJSON(self):
        flist = tkFileDialog.asksaveasfilename(title="Write captions file...",
                                             filetypes = [("JSON file", "*.json"),
                                                          ("All files", "*")],
                                             initialfile = "captions.json")
        if flist:  
...
class ConfigurationCard(Card):
    def __init__(self, parent, album):
        ...
        Entry(f, textvariable=self.galleryTitle).pack(side=RIGHT)
        f = Frame(f2); f.pack(expand=1, fill=X)
        Label(f, text="gallery sub-title:").pack(anchor=W, side=LEFT)        
        Entry(f, textvariable=self.gallerySubtitle).pack(side=RIGHT)
        f = Frame(f2); f.pack(expand=1, fill=X)
        Label(f,  text="gallery bottom line:").pack(anchor=W, side=LEFT)
        Entry(f, textvariable=self.galleryBottomLine).pack(side=RIGHT)
        ...
        Entry(f, textvariable=self.outputFolder).pack(side=LEFT)
...
class WebAlbumCreator(Tk):
    def __init__(self):
        ...
        self.cardStack.add("Images", 
                           lambda frame: ImagesCard(frame, self.album))
        self.cardStack.add("Captions",
