...
public class ControlPanelOverlay extends MouseListenerAdapter  
    implements PopupPanel.PositionCallback, ResizeListener {
  ...
  private class OverlayPopupPanel extends MovablePopupPanel {
    OverlayPopupPanel() {
      super();
      addStyleName("controlPanelOverlay");
  ...
  public static final EnumSet<Position> FREE;  
  public static final EnumSet<Position> CORNERS; 
  public static final EnumSet<Position> NORTH_AND_SOUTH;
  public static final EnumSet<Position> TOP_CAPTION;  
  public static final EnumSet<Position> BOTTOM_CAPTION;
  private static int MARGIN = 4;
  
  static {
    CORNERS = EnumSet.of(Position.NORTH_WEST, Position.NORTH_EAST, 
        Position.SOUTH_EAST, Position.SOUTH_WEST);
    NORTH_AND_SOUTH = EnumSet.complementOf(EnumSet.of(Position.WEST, Position.EAST));
    TOP_CAPTION = EnumSet.complementOf(EnumSet.of(Position.WEST, Position.EAST, Position.NORTH));
    BOTTOM_CAPTION = EnumSet.complementOf(EnumSet.of(Position.WEST, Position.EAST, Position.SOUTH));
    FREE = EnumSet.noneOf(Position.class);
  }
...
//  private ControlPanel       controlPanel;
  ...
  private int                locationX, locationY, baseWidth, baseHeight;
  private Timer              timer;
...
//    };
  ...
  /**
   * Disconnects the <code>ControlPanelOverlay</code> from the widget 
   * over which it is laid.
   */
  ...
  public void onResized() {
    ...
    if (popupVisible) {
      int oldBW = baseWidth;
      int oldBH = baseHeight;
      baseWidth = ((Widget) baseWidget).getOffsetWidth();
      baseHeight = ((Widget) baseWidget).getOffsetHeight();
      if (oldBW == 0) oldBW = baseWidth;
      if (oldBH == 0) oldBH = baseHeight;
      locationX = locationX * baseWidth / oldBW;
      locationY = locationY * baseHeight / oldBH;      
  ...
  public void setPosition(int offsetWidth, int offsetHeight) {
    ...
    if (possiblePositions.isEmpty()) {
      if (locationX > baseWidth - offsetWidth + MARGIN) {
        locationX = baseWidth - offsetWidth + MARGIN;
      } else if (locationX < MARGIN) {
        locationX = MARGIN;
      }
      
      if (locationY > baseHeight - offsetHeight + MARGIN) {
        locationY = baseHeight - offsetHeight + MARGIN;
      } else if (locationY < MARGIN) {
        locationY = MARGIN;
      }
      ...
      switch (popupPosition) {
        case NORTH: { 
          xpos += (baseWidth - offsetWidth) / 2;
          ypos += MARGIN;
          break;
        }
        case SOUTH: {
          xpos += (baseWidth - offsetWidth) / 2;
          ypos += baseHeight - offsetHeight - MARGIN;
          break;
        }
        case EAST: {
          xpos += baseWidth - offsetWidth - MARGIN;
          ypos += (baseHeight - offsetHeight) / 2;
          break;
        }
        case WEST: {
          xpos += MARGIN;
  ...
  public void setPossiblePositions(EnumSet<Position> allowed) {
    // now that free positioning is possible, don't: assert (!allowed.isEmpty());
