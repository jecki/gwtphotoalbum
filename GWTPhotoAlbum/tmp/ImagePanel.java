...
public class ImagePanel extends Composite implements HasClickHandlers, 
  ...
  private class ImageLoadHandler  implements LoadHandler {
    ...
    public void onLoad(LoadEvent event) {
      ...
      if (sender == active && sender != lastSender) {  // for some reason LOAD events are sometimes fired twice!?
        lastSender = sender;
  ...
  private   int           fading; // a negative value indicates that fading in and fading out should take place in sequence
  ...
  public ImagePanel(ImageCollectionInfo collection) {
    ...
    try {
      fading = Integer.parseInt(info.get("image fading"));
